# OXE DetentionAndTraining

- **Template ID:** `OXE_DetentionAndTraining`
- **Source file:** `analysis/templates/OXE_DetentionAndTrainingTemplate.docx`
- **Type:** Order
...
| `totalDetentionAndTrainingOrderPeriod` | prompt (value) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "totalDetentionAndTrainingOrderPeriod"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "totalDetentionAndTrainingOrderPeriod", then read .value |
