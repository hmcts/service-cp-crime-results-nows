# WEE CustodyWarrantOnExtraditionWithBailDirection

- **Template ID:** `WEE_CustodyWarrantOnExtraditionWithBailDirection`
- **Source file:** `analysis/templates/WEE_CustodyWarrantOnExtraditionWithBailDirectionTemplate.docx`
- **Type:** Warrant
- **Delivery:** Email
- **Language:** English
- **Code:** `WEE`
...
| `broughtBreach` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "broughtBreach"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "broughtBreach", then read .value |
| `broughtFts` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "broughtFts"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "broughtFts", then read .value |
