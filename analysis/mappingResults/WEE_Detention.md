...
- **Template ID:** `WEE_Detention`
- **Source file:** `analysis/templates/WEE_DetentionTemplate.docx`
- **Type:** Warrant
- **Delivery:** Email
- **Language:** English
- **Code:** `WEE`
...
| `durationValueForReturnToDetentionPeriod` | prompt (value) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "durationValueForReturnToDetentionPeriod"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "durationValueForReturnToDetentionPeriod", then read .value |
...
| `toBeServedAfterTheEndOfTheTermOfTheDetentionAndTrainingOrder` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "toBeServedAfterTheEndOfTheTermOfTheDetentionAndTrainingOrder"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "toBeServedAfterTheEndOfTheTermOfTheDetentionAndTrainingOrder", then read .value |
| `toBeServedBefore` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "toBeServedBefore"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "toBeServedBefore", then read .value |
| `toBeServedBeforeTheEndOfTheTermOfTheDetentionAndTrainingOrder` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "toBeServedBeforeTheEndOfTheTermOfTheDetentionAndTrainingOrder"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "toBeServedBeforeTheEndOfTheTermOfTheDetentionAndTrainingOrder", then read .value |
| `toBeServedConcurrently` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "toBeServedConcurrently"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "toBeServedConcurrently", then read .value |
