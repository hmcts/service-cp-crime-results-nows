# WEE CustodyWarrantOnExtraditionWithBailDirectionCategory2Territory

- **Template ID:** `WEE_CustodyWarrantOnExtraditionWithBailDirectionCategory2Territory`
- **Source file:** `analysis/templates/WEE_CustodyWarrantOnExtraditionWithBailDirectionCategory2TerritoryTemplate.docx`
- **Type:** Warrant
- **Delivery:** Email
- **Language:** English
- **Code:** `WEE`
...
| `bailConditionReason` | prompt (value) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "bailConditionReason"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "bailConditionReason", then read .value |
| `broughtBreachBail` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "broughtBreachBail"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "broughtBreachBail", then read .value |
| `broughtBSS` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "broughtBSS"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "broughtBSS", then read .value |
| `broughtWarrIss` | prompt (label) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "broughtWarrIss"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "broughtWarrIss", then read .value |
