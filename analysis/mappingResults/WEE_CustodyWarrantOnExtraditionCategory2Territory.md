# WEE CustodyWarrantOnExtraditionCategory2Territory

- **Template ID:** `WEE_CustodyWarrantOnExtraditionCategory2Territory`
- **Source file:** `analysis/templates/WEE_CustodyWarrantOnExtraditionCategory2TerritoryTemplate.docx`
- **Type:** Warrant
- **Delivery:** Email
- **Language:** English
- **Code:** `WEE`
...
| `remandedInCustody` | prompt (value) | Judicial result prompt | judicialResultPrompts[promptReference ≈ "remandedInCustody"].value / .label (matched case-insensitively) | Partial | no stable promptReference key exists here — would have to fuzzy-match offences[].results[].resultTexts[].label against a hardcoded display string for "remandedInCustody", then read .value |
